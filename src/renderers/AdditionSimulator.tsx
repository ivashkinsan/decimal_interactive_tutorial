...
const AdditionSimulator: React.FC = () => {
  ...
  const calculateAddition = (dec1: S21Decimal, dec2: S21Decimal) => {
    ...
    try {
      ...
      let finalSign = sign1;
      let operationType = 'ADD'; // Internal operation type

      if (sign1 !== sign2) {
        currentSteps.push({
          description: `Знаки разные. Операция сложения превращается в вычитание.`,
        });
        // A + (-B) = A - B
        // (-A) + B = B - A
        // Invert sign of d2 for internal subtraction logic
        setS21DecimalSign(d2, sign1); // Make signs temporarily same for mantissa op
        operationType = 'SUB';
      } else {
        currentSteps.push({
          description: `Знаки одинаковые. Будет выполнено сложение мантисс. Знак результата: ${finalSign === 0 ? '+' : '-'}`,
        });
      }
      ...
      setError(e.message || "Произошла ошибка во время сложения.");
