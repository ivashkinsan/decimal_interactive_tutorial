...
const SubtractionSimulator: React.FC = () => {
  ...
  const calculateSubtraction = useCallback((dec1: S21Decimal, dec2: S21Decimal) => {
    ...
    try {
      ...
      if (sign1 !== sign2) {
        currentSteps.push({ description: `После инверсии знаки разные. Операция сложения превращается в вычитание мантисс.` });
        operationType = 'SUB';
      } else {
        currentSteps.push({ description: `После инверсии знаки одинаковые. Будет выполнено сложение мантисс. Знак результата: ${finalSign === 0 ? '+' : '-'}` });
      }
      ...
      if (operationType === 'ADD') {
        ...
        if (getS21DecimalSign(mantissaResult) !== 0) {
            finalSign = finalSign === 0 ? 1 : 0;
            setS21DecimalSign(mantissaResult, 0);
        }
      }
      ...
      setSteps(currentSteps);
        ...
        if (e instanceof Error) {
            ...
            setError("Произошла неизвестная ошибка во время вычитания.");
        }
  ...
  }, [value1, value2, calculateSubtraction]);
