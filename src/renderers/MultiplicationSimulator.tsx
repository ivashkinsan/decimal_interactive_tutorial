...
const MultiplicationSimulator: React.FC = () => {
  ...
  const calculateMultiplication = (dec1: S21Decimal, dec2: S21Decimal) => {
    ...
    try {
      ...
      // 2. Сложение масштабов
      const scale1 = getS21DecimalScale(d1);
      const scale2 = getS21DecimalScale(d2);
      const finalScale = scale1 + scale2;
      currentSteps.push({
        description: `Сложение масштабов: ${scale1} + ${scale2} = ${finalScale}.`,
        highlight: 'scale1',
      });

      // 3. Умножение мантисс
      currentSteps.push({
        description: `Умножение мантисс Числа 1 и Числа 2.`,
      });
      const mantissaResult = mulMantissas(d1, d2);
      currentSteps.push({
        description: `Результат умножения мантисс: ${s21DecimalToNumber(mantissaResult)}`,
      ...
      setError(e.message || "Произошла ошибка во время умножения.");
